...
两种触发场景都归在同一个 `source` 下，用 `action` 字段区分：

| action | 触发条件 | 说明 |
|--------|----------|------|
| `profit_reduce` | 保证金收益每增加 10% | 逐步限价减持（10% → 减 10%，20% → 再减 20%，依此类推） |
| `profit_reduce_close` | 利润 > 12% 且持仓价值 < $100 | 小仓位利润保护，直接整体平仓，与利润回撤保护逻辑相同 |

> **注意**：`profit_drawdown`（利润回撤保护）与 `profit_reduce_close` 逻辑相似，合并在同一 source 下，不单独区分。
