...
type StrategyConfig struct {
	...
	// Deterministic K-line and volume gate applied only to new AI entries.
	TrendGate *TrendGateConfig `json:"trend_gate,omitempty"`
}

// TrendGateConfig controls deterministic single-symbol entry filtering.
type TrendGateConfig struct {
	Enabled           bool    `json:"enabled"`
	Timeframe         string  `json:"timeframe,omitempty"`
	Lookback          int     `json:"lookback,omitempty"`
	MinPriceChangePct float64 `json:"min_price_change_pct,omitempty"`
	MinVolumeRatio    float64 `json:"min_volume_ratio,omitempty"`
}
